...
function ProductItem({ product }) {
  ...
  return (
    <div className={styles['product-item']}>
      <Image loader={() => product.images[0]} src={product.images[0]} alt={`${product.title}`}  className={styles['product-item__img']} width="100%" height="100%" layout="responsive" />
      <div className={styles['item-info']}>
        <div className={styles['item-text']}>
          <p className={styles['item-text__price']}>${product.price}</p>
          <p className={styles['item-text__name']}>{product.title}</p>
        </div>
        <button onClick={() => handleClick(product)} className={styles['item-info__icon']}>
